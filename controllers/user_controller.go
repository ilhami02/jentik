...
func ScanImage(c *gin.Context) {
	...
	if hasilDeteksi.IsRawan {
		...
		query := `INSERT INTO reports (user_id, jenis_laporan, image_url, status, lokasi, created_at, updated_at) VALUES (?, 'jentik', ?, 'pending', ST_SetSRID(ST_MakePoint(?, ?), 4326), NOW(), NOW())`
